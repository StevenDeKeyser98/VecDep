...
#' where \eqn{h_{n} > 0} is a bandwidth parameter, \eqn{\widehat{U}_{ij}^{(\ell)} = \widehat{F}_{ij} (X_{ij}^{(\ell)})} with \eqn{\widehat{F}_{ij}(x_{ij}) = \frac{1}{n+1} \sum_{\ell = 1}^{n} 1 \left (X_{ij}^{(\ell)} \leq x_{ij} \right )} the (rescaled) empirical cdf of \eqn{X_{ij}}, and
...
#' h = hamse(input,pseudos = pseudos,n = n,estimator = "trans",bw_method = 1) # Local bandwidth selection
#' est_dens = transformationestimator(input,h,pseudos) # Gaussian transformation kernel estimator
#' true = copula::dCopula(rep(0.5,q), copula::normalCopula(0, dim = q)) # True density
#'
#' @export
